...
// 按段落边界切块，取消硬重叠（防止AI重复翻译），改为在提示词中传递上文
function splitIntoChunks(text, targetLen = 1500) {
...
for (let r = 0; r < mode.rounds; r++) {
  ...
  roundEl.innerHTML = `
    <div class="round-header round-toggle">
      ...
      <div class="round-title">第 ${r + 1} 轮迭代</div>
      <div class="round-badge" id="rbadge${r}">推演中</div>
    ...
    <div class="round-body" id="rbody${r}">
      <div class="paths-row">
        <div class="path-item"><div class="path-label"><span>A · 语言学家</span><span class="path-lock">并发</span></div><div class="path-text streaming" id="pa${r}"></div></div>
        <div class="path-item"><div class="path-label"><span>B · 本土编辑</span><span class="path-lock">并发</span></div><div class="path-text streaming" id="pb${r}"></div></div>
        <div class="path-item"><div class="path-label"><span>C · 领域专家</span><span class="path-lock">并发</span></div><div class="path-text streaming" id="pc${r}"></div></div>
        ...
        <div class="path-item path-item--implicit"><div class="path-label"><span>E · 隐语诠释者</span><span class="path-lock path-lock--implicit">后处理</span></div><div class="path-text streaming" id="pe${r}"></div></div>
        <div class="path-item path-item--style"><div class="path-label"><span>F · 风格镜像师</span><span class="path-lock path-lock--style">并发</span></div><div class="path-text streaming" id="pf${r}"></div></div>
      </div>
      <div class="critique-row">
        <div class="critique-item"><div class="critique-label">A 批判 B/C</div><div class="critique-text streaming" id="ca${r}"></div></div>
        <div class="critique-item"><div class="critique-label">B 批判 C/D</div><div class="critique-text streaming" id="cb${r}"></div></div>
        <div class="critique-item"><div class="critique-label">C 批判 D/F</div><div class="critique-text streaming" id="cc${r}"></div></div>
        <div class="critique-item"><div class="critique-label">D 批判 A/B</div><div class="critique-text streaming" id="cd${r}"></div></div>
        <div class="critique-item"><div class="critique-label">F 批判 A/C</div><div class="critique-text streaming" id="cf${r}"></div></div>
      </div>
      <div class="synth-row">
        <div class="synth-label"><span class="synth-label-text">多维综合裁决 (Round ${r + 1})</span><span class="synth-lock">裁决</span></div>
        <div class="synth-text streaming" id="synth${r}"></div>
      </div>
      <div class="memo-row" id="memo-row${r}" style="display:none">
        <div class="memo-label">迭代备忘录 (遗留问题 / 下轮策略)</div>
...
// \`\`\`
  ...
  if (r < mode.rounds - 1) {
    ...
    setTimeout(() => { body.style.maxHeight = '0px'; icon.classList.add('collapsed'); }, 600);
  }
}
...
let prevTranslation = ""; // 用于携带上文语境，避免脱节
...
// 强化分块 Prompt：强制要求输出为目标语言，绝不保留源语言
const userMsg = `作为纯粹的翻译器，请将下面的【待翻译片段】从【${src}】翻译成【${tgt}】。这是长文的第 ${i+1}/${total} 段。
// \`\`\`

要求：

1. 必须完全使用【${tgt}】输出，绝不允许原样照抄【${src}】原文！
1. 保持与上文风格一致，直接输出纯净译文正文。

${prevTranslation ? `【上文译文参考（仅供文脉连贯参考，不要翻译它）】\n${prevTranslation.slice(-400)}\n\n` : ''}【待翻译片段】\n${chunk}`;
...
// 强化合成裁决 Prompt
const chunkSynthSystem = `你是一个终极翻译裁决官。你的唯一任务是将输入的四路草稿合并优化，输出最终的纯净【${tgt}】译文。
// \`\`\`

【极度警告】：必须输出【${tgt}】，绝对不能输出任何【${src}】源语言文本！不准带任何前缀！`; const quickSynthMsg = `原文片段：\n${chunk}\n\n版本A：\n${resA}\n版本B：\n${resB}\n版本C：\n${resC}\n版本D：\n${resD}\n\n请裁决最优译文，直接输出纯净的【${tgt}】译文。`;
...
const { scores, remark } = parseAuditOutput(rawAudit);
if (document.getElementById('chunk_auditRemark').hasAttribute('data-has-reasoning')) {
document.getElementById('chunk_auditRemark').querySelector('.content-text').textContent = remark;
} else {
document.getElementById('chunk_auditRemark').textContent = remark;
}
