...
srcLang: LANGS[0],
tgtLang: LANGS[1],
rounds: Math.max(1, Math.min(5, parseInt(safeGet(‘local’, ‘prism_rounds’, ‘2’), 10) || 2)),
...
const globalName = { jszip: ‘JSZip’, mammoth: ‘mammoth’, xlsx: ‘XLSX’, pdfjs: ‘pdfjsLib’ }[name];
...
.replace(/\([{}])/g, ‘$1’).replace(/'([0-9a-fA-F]{2})/g, (*, h) => String.fromCharCode(parseInt(h, 16)))
.replace(/\u(-?\d+)\s*?/g, (*, c) => String.fromCharCode(+c)).replace(/[{}]/g, ‘’)
.replace(/\n{3,}/g, ‘\n\n’).trim();
}
...
: ‘’;
const remarkSnippet = item.remark
...
: ‘’;
const srcSnippet = escHtml(item.src.slice(0, 60)) + (item.src.length > 60 ? ‘…’ : ‘’);
const tgtSnippet = escHtml(item.tgt.slice(0, 60)) + (item.tgt.length > 60 ? ‘…’ : ‘’);
return ` <div class="history-item-meta"> <div class="history-langs">${item.srcCode} → ${item.tgtCode}</div> <div class="history-time">${item.time}</div> ${scoreChips} </div> <div class="history-item-content"> <div class="history-src">${srcSnippet}</div> <div class="history-tgt">${tgtSnippet}</div> ${remarkSnippet} </div> <div class="history-actions"> <button class="history-use-btn" data-id="${item.id}">使用</button> <button class="history-del-btn" data-id="${item.id}">删除</button> </div>`;
}

function attachHistoryItemEvents(el, item, onDelete) {
...
const newHistory = getHistory().filter(x => x.id !== item.id);
...
el.innerHTML = buildHistoryItemHTML(item);
attachHistoryItemEvents(el, item, () => renderHistoryList());
list.appendChild(el);
});
}
...
// 检测末尾未闭合的 ` if ((tail.match(/`/g) || []).length % 2 === 1) {
text += ‘`'; trailing.push('`’);
}
// 检测末尾未闭合的 
[（需要全局计数，因为链接文本可能很长）
...
if (!_markedLib || !raw) return escHtml(raw || ‘’);
try { return *markedLib.parse(raw); } catch(*) { return escHtml(raw); }
}
...
tags: [‘中→英’, ‘正式’], srcLang: ‘zh’, tgtLang: ‘en’,
...
演讲尾声，他引用了一句古希腊哲言：“认识你自己。“并补充道，“而在 AI 时代，我们更需要让 AI 认识每一个独特的你。”`}, { key: 'literature', icon: '📖', title: '文学经典', desc: '《小王子》法语原文，诗意与哲理', tags: ['法→中', '文学'], srcLang: 'fr', tgtLang: 'zh', text:`On ne voit bien qu’avec le cœur. L’essentiel est invisible pour les yeux.

Les hommes ont oublié cette vérité, dit le renard. Mais tu ne dois pas l’oublier. Tu deviens responsable pour toujours de ce que tu as apprivoisé. Tu es responsable de ta rose…

Je suis responsable de ma rose, répéta le petit prince, afin de se souvenir.`}, { key: 'scifi', icon: '🚀', title: '科幻巨著', desc: '《三体》经典片段，硬科幻风格', tags: ['中→英', '叙事'], srcLang: 'zh', tgtLang: 'en', text:`汪淼觉得，来找他的这四个人是一个奇怪的组合：两名警察和两名军人。如果那两个军人是武警还正常一些，但这是两名陆军军官。

汪淼第一眼就对来人没有好感。其实那名长得五大三粗的警官，让人家第一眼就喜欢的可能性也不大。另一名警官倒是很年轻，长的也挺帅。但汪淼一看就是那种少言寡语的人，从进到汪淼家开始，就没有说过一句话。

“汪淼？“那名粗壮的警察问。“是我。”“请跟我们走一趟。”`}, { key: 'techdoc', icon: '⚙️', title: '技术文档', desc: 'API 接口说明，术语密集', tags: ['英→中', '技术'], srcLang: 'en', tgtLang: 'zh', text:`The RequestRateLimiter GatewayFilter factory uses a RateLimiter implementation to determine if the current request is allowed to proceed. If not, it returns HTTP 429 - Too Many Requests status.
...
KeyResolver is a functional interface that allows you to derive the key for limiting requests. The default implementation uses the Principal name from ServerWebExchange. KeyResolver is a bean that implements the KeyResolver interface.`}, { key: 'business', icon: '💼', title: '商务信函', desc: '正式邮件，礼貌用语与格式', tags: ['英→中', '商务'], srcLang: 'en', tgtLang: 'zh', text:`Dear Dr. Chen,
...
Meridian Technologies AG`}, { key: 'poetry', icon: '🏮', title: '古典诗词', desc: '唐诗宋词，意境深远', tags: ['中→英', '文学'], srcLang: 'zh', tgtLang: 'en', text:`静夜思
李白
...
不知天上宫阙，今夕是何年。
我欲乘风归去，又恐琼楼玉宇，高处不胜寒。
起舞弄清影，何似在人间。`}, { key: 'philosophy', icon: '🏛️', title: '哲学思辨', desc: '尼采片段，抽象深邃', tags: ['德→中', '哲学'], srcLang: 'de', tgtLang: 'zh', text:`Wer mit Ungeheuern kämpft, mag zusehn, dass er nicht dabei zum Ungeheuer wird. Und wenn du lange in einen Abgrund blickst, blickt der Abgrund auch in dich hinein.

Es ist immer etwas Wahnsinn in der Liebe. Es ist aber auch immer etwas Vernunft im Wahnsinn.

Der Mensch ist ein Seil, geknüpft zwischen Tier und Übermensch — ein Seil über einem Abgrunde.`}, { key: 'multilang', icon: '🌐', title: '多语混杂', desc: '日韩英混排，测试语言检测', tags: ['混合', '检测'], srcLang: 'ja', tgtLang: 'zh', text:`AI 技術の発展は私たちの生活を大きく変えました。特に 번역 분야에서 혁명적인 변화가 일어났습니다.
...
grid.innerHTML = DEMO_LIBRARY.map(d => `<button class="demo-card" data-key="${d.key}" style="display:flex;flex-direction:column;align-items:flex-start;text-align:left;padding:14px;border:1.5px solid var(--border-cream);border-radius:var(--r-lg);background:var(--ivory);cursor:pointer;transition:all 0.2s;gap:6px;position:relative;overflow:hidden;">

<div style="font-size:22px;margin-bottom:2px;">${d.icon}</div>
<div style="font-size:13px;font-weight:600;color:var(--near-black);font-family:var(--sans);">${d.title}</div>
<div style="font-size:10px;color:var(--stone);line-height:1.4;">${d.desc}</div>
<div style="display:flex;gap:4px;margin-top:4px;flex-wrap:wrap;">${d.tags.map(t => `<span style="font-size:9px;padding:1px 6px;border-radius:var(--r-full);background:var(--warm-sand);color:var(--olive);font-family:var(--mono);">${t}</span>`).join('')}</div>
...
card.addEventListener('mouseenter', () => { card.style.borderColor = 'var(--terracotta)'; card.style.transform = 'translateY(-2px)'; card.style.boxShadow = 'var(--shadow-md)'; card.querySelector('.demo-card-corner').style.opacity = '0.8'; });
card.addEventListener('mouseleave', () => { card.style.borderColor = 'var(--border-cream)'; card.style.transform = ''; card.style.boxShadow = ''; card.querySelector('.demo-card-corner').style.opacity = '0'; });
...
function hideDemoPanel() { document.getElementById('demoPanelModal').style.display = 'none'; }
...
const roundTokens = rd.usageTokens?.total
? `输入 ${rd.usageTokens.prompt?.toLocaleString() || '?'} / 输出 ${rd.usageTokens.completion?.toLocaleString() || '?'} / 总计 ${rd.usageTokens.total.toLocaleString()}`
: ‘统计中…’;
md += `### 第 ${rd.round} 轮\n\n> **Token 消耗：** ${roundTokens}\n\n`;
...
${state.customPrompt ? `【最高优先级 — 用户特殊要求】
...
const numEl = document.getElementById(`${idPrefix}s${i}`);
const spEl  = document.getElementById(`sp${i}`);
if (numEl) numEl.textContent = ‘?’;
if (spEl)  { spEl.textContent = `${label} ?`; spEl.classList.add(‘loaded’); }
});
return;
}
scores.forEach((s, i) => {
const numEl = document.getElementById(`${idPrefix}s${i}`);
const itemEl = document.getElementById(`${idPrefix}si${i}`);
const barEl  = document.getElementById(`${idPrefix}sb${i}`);
const spEl   = document.getElementById(`sp${i}`);
if (numEl) numEl.textContent = s;
if (s >= 9) {
numEl?.classList.add(‘excellent’);
itemEl?.classList.add(‘excellent’);
barEl?.classList.add(‘excellent’);
}
if (barEl) {
setTimeout(() => {
barEl.style.width = (s * 10) + ‘%’;
barEl.style.transition = ‘width 0.9s cubic-bezier(0.2,1,0.2,1)’;
}, i * 180);
}
if (spEl) { spEl.textContent = `${scoreLabels[i]} ${s}`; spEl.classList.add(‘loaded’); }
});
}
...
const sysPrompt = `你是一位资深翻译专家。任务：将${escHtml(src)}文本精准翻译为${escHtml(tgt)}。 要求：忠实原文、语言流畅、地道自然。必须直接输出译文正文，绝对不要带任何前缀标签、分析过程或元信息。`;
...
list.innerHTML = ‘<div class="history-empty">’ + (q ? ‘无匹配的历史记录’ : ‘暂无翻译历史’) + ‘</div>’;
return;
}
list.innerHTML = ‘’;
...
_fileQueueIndex++;
// 首个文件：正常加载（覆盖模式）
// 后续文件：需要追加到现有内容后，先保存当前内容再追加
const prevText = _fileQueueIndex > 1 ? document.getElementById(‘sourceText’).value : null;
await handleFileSelect(file);
if (prevText !== null) {
// handleFileSelect 会覆写 sourceText，在此追加前次内容
const newContent = document.getElementById(‘sourceText’).value;
document.getElementById(‘sourceText’).value = prevText + ‘\n\n—\n\n’ + newContent;
updateWordStats();
updateTranslateBtnState();
}
if (_fileQueueIndex < _fileQueue.length) {
